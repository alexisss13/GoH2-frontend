...
// (Tus componentes de Iconos se mantienen igual, puedes dejarlos como estaban)
...
function LoginComponent() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    // 1. Ejecutamos el login y esperamos true/false
    const success = await handleLogin(data);

    if (success) {
      const token = useAuthStore.getState().token;
      // 2. Leemos el callbackUrl
      const callbackUrl = searchParams.get('callbackUrl');

      if (callbackUrl && token) {
        // CASO A: Volver a la Landing Page (Checkout)
        const targetUrl = decodeURIComponent(callbackUrl);
        const separator = targetUrl.includes('?') ? '&' : '?';
        // Redirección externa forzada
        window.location.href = `${targetUrl}${separator}token=${token}`;
      } else {
        // CASO B: Ir al Dashboard
  ...
  return (
    <div className="relative min-h-screen bg-black text-white overflow-hidden">
      <Header />
      {showNotification && messageKey && <Notification messageKey={messageKey} type="success" onClose={handleCloseNotification} />}
        ...
        <div className="flex-1 flex flex-col items-center md:items-start justify-center w-full">
          <div className="w-full max-w-md">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                <Input icon={<EmailIcon />} placeholder={t('email')} type="email" {...register('email')} isInvalid={!!errors.email} aria-invalid={!!errors.email} />
                <FormError message={errors.email?.message} />
              </div>
              <div>
                <PasswordInput placeholder={t('password')} {...register('password')} isInvalid={!!errors.password} aria-invalid={!!errors.password} />
                <FormError message={errors.password?.message} />
                <div className="w-full text-right mt-2">
                  <Link href="/restablecer-password" className="text-sm text-gray-light hover:text-white transition-colors underline">
                    {t('forgotPasswordTitle')}
                  </Link>
                </div>
              </div>
              {error && <p role="alert" className="flex items-center justify-center text-red-500 text-sm text-center py-2"><ErrorIcon /> {error}</p>}
              ...
              <div className="text-center pt-4 text-gray-light">
                  {t('dontHaveAccount')}{' '}
                  <Link href={`/registro?${searchParams.toString()}`} className="text-primary hover:text-primary/80 font-semibold">
                    {t('registerLink')}
                  </Link>
              </div>
            </form>
