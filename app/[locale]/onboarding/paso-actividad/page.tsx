...
export default function OnboardingActivityPage() {
  ...
  const [percentage, setPercentage] = useState(62.5);
  const targetPercentage = 75;
  const [displayedText, setDisplayedText] = useState('');
  const fullText = tOnboarding('activityPrompt');

  useEffect(() => {
    const duration = 1000;
    const steps = 60;
    const increment = (targetPercentage - 62.5) / steps;
  ...
  // Obtener las opciones de actividad
  const activityOptions = tOnboarding.raw('activityOptions') as Record<string, string>;
  ...
  return (
    <div className="relative min-h-screen bg-black text-white overflow-hidden">
          ...
          <div className="max-w-2xl mx-auto">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-gray-light">
                {tOnboarding('step', { current: 6, total: 8 })}
              </span>
            ...
            <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
              <div className="h-full bg-primary rounded-full animate-progress-step-6" style={{ width: '75%' }} />
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center md:items-start justify-center w-full">
          <div className="w-full max-w-md space-y-6">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div>
                <label htmlFor="nivelActividad" className="text-gray-light mb-2 text-sm block">
              ...
              <div className="bg-gray-medium/10 rounded-xl p-4 text-sm text-gray-light border border-gray-800">
                <p className="mb-3 font-semibold text-white text-sm flex items-center gap-2">
                  <span className="text-base">💡</span>
                  {tOnboarding('activityHelpTitle')}
                </p>
                <ul className="space-y-1.5 text-xs">
                  <li className="flex items-start gap-2">
                    <span className="text-primary mt-0.5">•</span>
                    <span>
                      <strong className="text-white">{activityOptions.Sedentario}:</strong> {tOnboarding('activitySedentary')}
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-primary mt-0.5">•</span>
                    <span>
                      <strong className="text-white">{activityOptions.Ligero}:</strong> {tOnboarding('activityLight')}
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-primary mt-0.5">•</span>
                    <span>
                      <strong className="text-white">{activityOptions.Moderado}:</strong> {tOnboarding('activityModerate')}
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-primary mt-0.5">•</span>
                    <span>
                      <strong className="text-white">{activityOptions.Activo}:</strong> {tOnboarding('activityActive')}
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-primary mt-0.5">•</span>
                    <span>
                      <strong className="text-white">{activityOptions.MuyActivo}:</strong> {tOnboarding('activityVeryActive')}
                    </span>
                  </li>
                </ul>
              </div>
      ...
      <style jsx>{`
        ...
        .animate-progress-step-6 {
          animation: progress-step-6 1s ease-out forwards;
        }
      `}</style>
    </div>
  );
