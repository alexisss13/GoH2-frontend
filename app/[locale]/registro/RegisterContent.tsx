...
function RegisterComponent() {
  ...
  // (Efecto de barra de progreso igual)
  useEffect(() => {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    // 1. Ejecutamos registro y esperamos true/false
      ...
      <div className="flex flex-col md:flex-row min-h-screen items-center justify-center px-8 pt-24 pb-24 gap-8 md:gap-12 max-w-7xl mx-auto">
        ...
        <div className="absolute top-20 left-0 right-0 px-8 z-10">
          <div className="max-w-2xl mx-auto">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-gray-light">{tOnboarding('step', { current: 3, total: 8 })}</span>
              <span className="text-xs text-primary font-semibold">{percentage.toFixed(1)}%</span>
            </div>
        ...
        <div className="flex-1 flex flex-col items-center md:items-start justify-center w-full animate-fade-in">
          <div className="w-full max-w-md">
            <div className="mb-6">
              <h1 className="text-3xl lg:text-4xl font-bold text-white mb-1 text-center md:text-left">{t('registerTitle')}</h1>
            </div>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5">
              ...
              <div className="animate-fade-in-delayed">
                <Button type="submit" variant="primary" isLoading={isLoading} className="mt-5 w-full">{t('registerButton')}</Button>
              </div>
              <div className="text-center pt-4 text-gray-light animate-fade-in-delayed">
                  {t('alreadyHaveAccount')}{' '}
                  <Link href={`/login?${searchParams.toString()}`} className="text-primary hover:text-primary/80 font-semibold">{t('loginLink')}</Link>
              </div>
            </form>
          </div>
        </div>
